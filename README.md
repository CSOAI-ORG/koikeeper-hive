...
```
L7  PRESENTATION    koi gold + pond jade (Open Design)
L6  ORCHESTRATION   Hermes sub-context (Kimi K2.6 / DeepSeek V3.5 / local)
L5  DOMAIN MCP      fishkeeper-ai-mcp, k25-vision
L4  AGENT MEMORY    agentmemory (v0.9.26) — isolated mode
L3  KNOWLEDGE GRAPH Cognee subgraph — scope: koi varieties, water quality (pH, NH3, NO2), breeding, image-based diagnostics
L2  VERSIONED HIST  Memoria (v0.4.0) — namespace "koikeeper"
...
```bash
git clone https://github.com/CSOAI-ORG/koikeeper-hive
cd koikeeper-hive
...
`https://koikeeper.ai/.well-known/agent-card.json` — see `agent-card.json`.
...
£199/mo premium tier. Niche, affluent, competition is KoiQuanta.
